...
export const Button: React.FC<ButtonProps> = ({
  ...
  const baseClasses =
    "whitespace-nowrap cursor-pointer inline-flex items-center justify-center rounded-lg font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300 disabled:cursor-not-allowed";
  ...
  return (
    <button className={finalClassName} disabled={disabled} {...props}>
      {iconL && (
        <span className="flex items-center justify-center">{iconL}</span>
      )}
      <span className="min-w-fit min-h-fit">{children}</span>
      {iconR && (
        <span className="flex items-center justify-center">{iconR}</span>
      )}
    </button>
  );
};
